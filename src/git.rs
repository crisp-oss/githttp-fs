...
impl GitOrder {
    ...
    /// `implicit_allow_hidden_files` decides whether dot-prefixed siblings are
    /// folded in at all; off, they are left out of the generated index (a hidden
    /// entry the index already names is still kept — see
    /// [`Self::materialized_order`]). Whether the *positioned* entry may itself
    /// be hidden is settled at the route, which is where a caller-facing `400`
    /// belongs.
    ///
    ...
    pub fn reorder_entry(
        ...
        implicit_allow_hidden_files: bool,
    ...
    ///
    /// `allow_hidden_files` decides whether dot-prefixed siblings are folded in.
    /// Off (the default at the route) they are left out, so materialising an
    /// index does not quietly pin the metadata a repository happens to carry —
    /// the same judgement `include_hidden_files=false` makes on a listing, and
    /// the reason it is the *default* here too: this index is generated by the
    /// server from a directory, not dictated entry by entry. A hidden entry the
    /// stored index **already names** is kept regardless: it was pinned
    /// deliberately (through this flag, or `allow_hidden_files` on `PUT /order`),
    /// and positioning an unrelated file is no occasion to unpin it.
    ///
    ...
    fn materialized_order(
        ...
        if !allow_hidden_files {
            entries.retain(|entry| {
                !order::is_hidden(entry) || ranks.contains_key(order::entry_name(entry))
            });
        }
    ...
    /// itself is skipped: it is not an entry of the directory it orders.
    /// Anything that is neither a blob nor a tree (a submodule) is left out,
    /// exactly as it is everywhere else in this API. Hidden entries are returned
    /// here and filtered by the caller, which is the only place that knows
    /// whether one was already pinned.
