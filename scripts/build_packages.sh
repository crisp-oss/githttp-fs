...
pushd "$BASE_DIR" > /dev/null
    echo "Executing packages build steps for githttp-fs..."
    ...
    if [ $rc -eq 0 ]; then
        echo "Success: Done executing packages build steps for githttp-fs"
    else
        echo "Error: Failed executing packages build steps for githttp-fs"
    fi
popd > /dev/null
