...
| `GET` | `/v1/:collection_id/:tenant_id/files?prefix_path=&maximum_depth=&include_hidden_files=&file_name_starts_with=&file_paths=&include_date_from=&include_date_to=&include_date_type=&apply_order_index=&implicit_order_default_index=&page=&per_page=` | List tracked files as a tree; optional `apply_order_index` (default `false`) orders every level by that directory's stored file order index, and optional `implicit_order_default_index` (a number, unset by default) sets the index an entry the order index does not name is treated as holding — `0` or `-1` lifts every unordered entry *above* the ordered ones, unset leaves them behind them; optional `prefix_path` scopes the listing to a sub-directory (e.g. `?prefix_path=/docs`); optional `maximum_depth` limits how many directory levels deep the listing goes; optional `include_hidden_files` (default `false`) includes dot-prefixed entries; optional `file_name_starts_with` narrows the listing to files *and directories* whose leaf name begins with the given prefix, case-insensitively (a matched directory brings its contents along); it accepts either a bare string or a JSON-array string of prefixes (e.g. `?file_name_starts_with=["intro", "readme"]`), matching an entry whose name begins with *any* of them; optional `file_paths` selects exactly the paths it names instead (same two spellings, each path relative to `prefix_path`, a path the repository does not hold simply absent from the result) and must not be combined with `file_name_starts_with`; optional `include_date_from`/`include_date_to` (RFC 3339 date-times) narrow the listing to files whose git date falls in the half-open window `[from, to)`, and `include_date_type` (`updated` default, or `created`) selects which date is compared; `page`/`per_page` paginate over the root-level entries of the listing (default 100, max 500) |
...
`partial` says whether entries are missing from the answer. It is `false` on every listing but one: a date-filtered listing whose history walk spent the `limits.date_filter_maximum_ms` safety timer (ten seconds by default — see below). Read it as the opposite of `has_more`: `has_more` promises there is another page to ask for, `partial` warns that asking again changes nothing — the answer is short because the server stopped looking, and only a narrower request or a larger budget will fill it in.
...
The optional `file_paths` query parameter narrows the listing to **exactly the paths it names**, and is the answer to "give me these entries, whichever of them exist". It accepts the same two spellings as `file_name_starts_with`: a bare string (a single path), or a JSON-array string of paths (e.g. `?file_paths=["intro.md", "guides/setup.md"]`, URL-encoded). Each path is **relative to `prefix_path`** when one is set — the caller spells them against the listing root it asked for, which is also the root the response reports them against — and each is sanitised exactly as the read route's `*path` is (leading slashes stripped, `..`/`.`/`.git` components rejected with `400`), with a trailing slash tolerated so a folder may be spelled `guides/`. An empty value, an empty array, and an empty path are all `400`. Duplicates are *not* rejected, unlike on the batch read route: a listing is a tree built by name rather than an index-aligned array, so naming the same path twice asks a question with one answer.

**A path the repository does not hold is left out of the result, not an error.** The parameter states what the caller is interested in, so a listing of three paths where one was deleted answers `200` with the other two, and a request naming nothing that exists answers `200` with an empty `files` array — never `404`. That is what makes it usable as a bulk existence check: what comes back is what is there.

Otherwise it behaves exactly like `file_name_starts_with`, being the same listing mode reached by a different predicate — an entry is selected by its exact path instead of its leaf name. A named **file** is returned as a leaf, its ancestor directories present purely as the structure leading to it; a named **directory** is returned with its whole subtree expanded, so naming a folder reads it in one call, and it still shows (childless) when everything inside it is filtered out. Hidden entries stay excluded unless `include_hidden_files=true`, and they are gated by *path* rather than by leaf name — naming `.drafts/intro.md` is not a way around the flag. `maximum_depth` bounds the result the same way: a named path deeper than the limit is not returned at all, and a named directory sitting *at* the limit renders as the childless stub the plain listing shows. An order index is never selectable (`?file_paths=[".order.json"]` returns nothing, whatever `include_hidden_files` says), a date window intersects with the selection, `apply_order_index` orders the result as everywhere else, and pagination stays parent-based over the root-level entries of the selected tree. No blob is ever opened — and this is the one listing mode that walks nothing it was not asked for, costing one tree lookup per path.

`file_paths` and `file_name_starts_with` **must not be combined**: they are two ways of selecting over the same tree, and a request carrying both would only ever intersect one with the other, hiding a mistyped path behind an empty result. Passing both is a `400`.

The optional `include_date_from` / `include_date_to` query parameters narrow the listing to files whose git date falls inside the half-open window `[from, to)` — `include_date_from` inclusive, `include_date_to` exclusive — each an RFC 3339 date-time (e.g. `2026-06-16T10:00:00Z`), strictly validated (any other spelling returns `400`). Each bound is independently optional (an open-ended range); when both are given, `from` must be strictly before `to` (equal bounds select nothing, so it is a `400`). The optional `include_date_type` selects which date is compared: `updated` (the default) is the most recent commit that touched the file, `created` is the oldest commit that introduced it under its current path (renames are *not* followed). `include_date_type` is always validated against those two values, but the date filter is only active — and its cost only paid — when at least one bound is present; passing `include_date_type` alone changes nothing and keeps the cheap tree-only fast path. This is the crucial caveat: unlike every other listing mode, a date filter cannot be answered from tree objects (a tree entry carries no timestamp), so it walks commit history. The walk is still blob-free (it compares tree/oid deltas per commit, with no patch, stats, or rename detection) but its cost scales with history length, not page size — `updated` stops as soon as every in-scope file has been dated, whereas `created` must reach the root of history. `prefix_path` narrows that walk as well as the listing: each commit is diffed only inside the prefix, so a repository whose history spreads over many sibling directories (one per locale, say) does not pay for the ones the listing never looks at. It composes with the other parameters: `prefix_path` and `include_hidden_files` scope which files are candidates, and `file_name_starts_with` intersects with it (a file must match both the name prefix and the date window). Directories are kept only as the structure leading to a surviving file, so an emptied directory is pruned. `maximum_depth` bounds what is *returned* rather than what is looked at: a directory sitting at the limit comes back as the same childless stub the plain listing returns, but only when the subtree below it holds an in-window file — so a caller browsing one level at a time still has folders to descend into, and a folder with nothing in the window is not offered. Deciding that means walking past the limit even though nothing below it is rendered, which is why a depth-limited date filter costs what an unlimited one costs. The response shape is unchanged — the filter only removes entries; per-file dates are not reported.

Because that walk is the one cost on this route that the request cannot bound, it runs under a safety timer: `limits.date_filter_maximum_ms`, **ten seconds by default**. A listing that exhausts it stops walking and answers `200` with what it had dated by then, `partial: true` — a short answer, deliberately, rather than an error or an unbounded wait. Setting the key to `0` turns the timer off and restores the unbounded walk, whose answer is always exact. Files whose date was never read are simply absent, so the answer is a subset of the true one; with `include_date_type=created` it can also be *wrong* rather than merely short, since a created date is only final once the walk has reached the root of history, and a file added, deleted and added again may be left holding the later of its two additions. `has_more` still describes the page rather than the walk, so a partial answer can report `has_more: false` while entries the window holds are missing. The timer is node-wide and not overridable per request: it is an operator's guard rail against one expensive query, not something a caller chooses.
