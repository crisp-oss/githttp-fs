...
- **A replica mirrors its working tree too, by checkout rather than file by file** — nothing on a replica reads the working tree (every route answers from HEAD's tree and the object database), but a master keeps its files on disk so a human can `ls` a tenant, and a replica that did not would make the same content visible on one node and invisible on the other, with `git status` reporting every file as deleted. So a landed pack checks the repository out to its new HEAD, forcing over whatever is there and removing what HEAD no longer names — a replica's working tree is wholly server-owned, so anything untracked is a file the upstream deleted. A *checkout* rather than a diff of the two commits, which would be the cheaper mirror the write path uses: a checkout converges from any prior state, including the no-working-tree state a store replicated by an older build is in, where a delta would faithfully apply one commit onto a tree that was never correct. That same property is why a replica booting with repositories checks all of them out once before it starts following, and why the pass is never repeated afterwards. It is logged and swallowed on failure, like every other working-tree mirror in this codebase: HEAD is authoritative, so a checkout that cannot run must not turn a correctly applied pack into a sync that retries forever.
