...
| `GET` | `/v1/:collection_id/:tenant_id/files?prefix_path=&maximum_depth=&include_hidden_files=&file_name_starts_with=&page=&per_page=` | List tracked files as a tree; optional `prefix_path` scopes the listing to a sub-directory (e.g. `?prefix_path=/docs`); optional `maximum_depth` limits how many directory levels deep the listing goes; optional `include_hidden_files` (default `false`) includes dot-prefixed entries; optional `file_name_starts_with` narrows the listing to files *and directories* whose leaf name begins with the given string, case-insensitively (a matched directory brings its contents along); `page`/`per_page` paginate over the root-level entries of the listing (default 100, max 500) |
...
- **File listing never opens blobs** — the tree endpoint is served from git tree objects alone (no sizes are reported), and pagination over root-level entries is decided before any subtree is opened, so off-page directories are never walked. The sole exception is `file_name_starts_with` search: matches can be nested anywhere, so that mode walks the whole in-scope tree (still tree objects only, no blob) before paginating over the *matched* tree's root-level entries; the match is a case-insensitive prefix test on each entry's leaf name — files *and* directories, a matched directory carrying its whole subtree — and directories holding no match are pruned. The count endpoint (`GET .../count/files`) shares the same contract: it walks tree objects only, honouring the listing's `prefix_path`/`maximum_depth`/`include_hidden_files` semantics, with an optional `restrict_file_extensions` filter matched against entry names.
