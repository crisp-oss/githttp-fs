...
- **File listing never opens blobs** — the tree endpoint is served from git tree objects alone (no sizes are reported), and pagination over root-level entries is decided before any subtree is opened, so off-page directories are never walked. The count endpoint (`GET .../count/files`) shares the same contract: it walks tree objects only, honouring the listing's `prefix_path`/`maximum_depth`/`include_hidden_files` semantics, with an optional `restrict_file_extensions` filter matched against entry names.
